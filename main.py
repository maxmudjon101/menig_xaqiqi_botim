...
ADMINS = [2110736070]
# Bot token
API_TOKEN="6593266052:AAEurCJvQt0YOxbQiu49NkQW3JArVvYJevg"
        ...
        if value == 'True':
            ...
            for channel in channels:
                try:
                    ...
                    status = await bot.get_chat_member(channel_info.id, user_id)

                except Exception as e:
                    continue
                if status.status not in _:
                    keyboard.add(types.InlineKeyboardButton(text=f'{channel_info.title}', url=f'{channel_info.invite_link}'))
                    checking = True
            if checking:
                button = "A'zolikni tasdiqlash 🔅"
                keyboard.add(types.InlineKeyboardButton(text=button, callback_data='check_channel'))
...
async def channel_check_handler(callback: types.CallbackQuery):
    ...
    if check:
        ...
        await callback.answer("Obuna bo'lganingiz uchun rahmat ")
    else:
